...
def ai_processing_worker():
    ...
    while is_running:
        if latest_frame is not None:
            ...
            # -------------------------------------
            # TASK 3: Currency Detection (RIGHT SIDE)
            # -------------------------------------
            # Shifted the bounding box logic to the right side (65% to 95% of screen width)
            cx1, cy1 = int(w * 0.65), int(h * 0.2)
            cx2, cy2 = int(w * 0.95), int(h * 0.8)
...
def generate_frames():
    ...
    while True:
        ...
        if not success:
            ...
            # =====================================
            # 1. UPPER SIDE: Object Detection Panel
            # =====================================
            cv2.rectangle(frame, (0, 0), (w, 40), (0, 0, 0), -1)
            
            # Get unique object names for the top panel display
            unique_objs = list(set([obj[0] for obj in detected_objects]))
            obj_text_display = " | ".join(unique_objs) if unique_objs else "Scanning objects..."
            cv2.putText(frame, f"OBJECTS DETECTED: {obj_text_display.upper()}", (20, 28), cv2.FONT_HERSHEY_DUPLEX, 0.7, (0, 215, 255), 2)

            # Draw Object Boxes (so you can visually see where they are)
            ...
            # =====================================
            # 2. RIGHT SIDE: Currency Scanner Box
            # =====================================
            cx1, cy1 = int(w * 0.65), int(h * 0.2)
            cx2, cy2 = int(w * 0.95), int(h * 0.8)
            ...
            if detected_currency:
                ...
                text = f"TAKA: {curr_label}"
                cv2.rectangle(frame, (cx1, cy2), (cx2, cy2 + 35), (0, 255, 255), -1)
                cv2.putText(frame, text, (cx1 + 5, cy2 + 25), cv2.FONT_HERSHEY_DUPLEX, 0.7, (0, 0, 0), 2)

            # =====================================
            # 3. BOTTOM SIDE: Live Text Reader Panel
            # =====================================
